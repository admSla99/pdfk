...
def cmd_build(a) -> int:
    ...
    same = existing and existing.pdf_sha256 == sha and existing.page_range == (list(page_range) if page_range else None) and (pack / "docling.json").is_file()
    if same and not a.force:
        print(f"{doc_id}: docling.json is up to date (same PDF hash); running post-processing only. Use --force to reconvert.")
    else:
        print(f"{doc_id}: converting {pdf.name}{' pages ' + a.pages if a.pages else ''} … (this can take a while)", flush=True)
        stats = convert_pdf(
            pdf, pack / "docling.json", page_range=page_range, ocr=a.ocr, figures=a.figures,
            threads=a.threads, device=a.device, artifacts_path=a.artifacts_path, table_mode=a.table_mode,
        )
        entry = DocEntry(
            id=doc_id, source=str(pdf), pdf_sha256=sha, pages=stats.pages,
...
def _status_line(e: DocEntry) -> str:
    ...
    return (f"{e.id}: \"{e.title}\" {e.pages}p{pr}, {c.get('sections', 0)} section files, {c.get('headings', 0)} headings, "
            f"{c.get('tables', 0)} tables, {c.get('registers', 0)} registers; confidence mean={e.grades.get('mean', '?')} "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument("--figures", action="store_true", help="export picture images")
    ...
    s.add_argument("--doc")
    s.add_argument("--kind", choices=["heading", "text", "list", "table", "code"], help="restrict block kind")
